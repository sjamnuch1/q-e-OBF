...
!----------------------------------------------------------------------------
  ...
  USE qexsd_copy,      ONLY : qexsd_copy_parallel_info, &
       ...
       qexsd_copy_kpoints
  USE io_global, ONLY : ionode, ionode_id, stdout !SJ added stdout
  ...
  PRIVATE
  PUBLIC :: pw_write_schema, write_collected_wfc, write_collected_wfc_simple
  PUBLIC :: write_collected_wfc_nkp
...
!-------------------------------------------------------------------------------
    ...
    !
    !------------------------------------------------------------------------
    SUBROUTINE write_collected_wfc_simple( )
      !------------------------------------------------------------------------
      !
      ...
      USE klist,                ONLY : nks, nkstot, xk, ngk, igk_k !ngk = #PW for each kpt
      ...
      !SJ
      USE wfc_basis,            ONLY : wfc0
      USE input_simple,         ONLY : npw_max, ntot_e
      USE gvecw,                ONLY : gcutw
      USE pwcom,                ONLY : init_igk_simple
      !
      IMPLICIT NONE
      !
      ...
      !SJ Above commands are for CUDA/GPU stuff. CPU version will do nothing.
      dirname = restart_dir ()
      !
      ...
      WRITE(stdout,*) 'npw_max from simple', npw_max
      WRITE(stdout,*) 'original npwx',npwx
      WRITE(stdout,*) 'Optimal basis from simple.x, universal representative for all kpoints'
      WRITE(stdout,*) 'Determining new ecut'
      !WRITE(stdout,*) 'nks'   ,nks
      !WRITE(stdout,*) 'size of ngk', size(ngk)

      !WRITE(stdout,*) 'Calling init_igk(npw_max,ngm,g,gcutw) for simple'
      !WRITE(stdout,*) 'ngm',ngm
      !WRITE(stdout,*) 'ngk', ngk(1:nks)
      !CALL init_igk_simple(npw_max,ngm,g,gcutw)
      ...
      !
      ALLOCATE( ngk_g( nks ) ) !nks = #kpt in pool
      ngk_g(1:nks) = ngk(1:nks) !Might need to modify ngk to simple PW 
      WRITE(stdout,*) 'Size of ngk',size(ngk) 
      ...
      !
      ALLOCATE ( igk_l2g_kdip( npw_max ) )
      !
      ALLOCATE ( mill_k( 3, npw_max ) )

      WRITE(stdout,*) 'Size of mill(:,:)', size(mill,1),size(mill,2),size(mill)
      WRITE(stdout,*) 'Size of ig_l2g',size(ig_l2g)
      !
      !WRITE(stdout,*) 'nks',nks
      k_points_loop: DO ik = 1, 1
         !SJ OB is for all the kpoints. 
         !SJ To get wavefunction at specific kpoint with wavevector k
         !SJ u_nk(x) = summation [1 -> ntot_e ] of [b_i_{nk}* b_i(x) ]
         WRITE(stdout,*) 'Working on kpoint ik:', ik
         !
         ...
         !
         npw_g = MAXVAL( igk_l2g(1:npw_max)) !ngk(ik)) )
         WRITE(stdout,*) 'npw_g',npw_g
         WRITE(stdout,*) 'ngk_g(ik)', ngk_g(ik)

         CALL mp_max( npw_g, intra_pool_comm )
         !
         igk_l2g_kdip = 0
         WRITE(stdout,*) 'npw_g after mp_max',npw_g
         WRITE(stdout,*) 'Calling gk_l2gmap_kdip)'

         CALL gk_l2gmap_kdip( npw_g, ngk_g(ik), npw_max, igk_l2g, & !npw_max replaced ngk(ik)
                              igk_l2g_kdip )
         !
         ! ... mill_k(:,i) contains Miller indices for (k+G)_i
         !
         DO ig = 1, npw_max !ngk (ik)
            mill_k(:,ig) = mill(:,igk_k(ig,ik)) !SJ might need to get new mill(:,:) for simple? No.
         END DO
         !
         ! ... read wavefunctions - do not read if already in memory (nsk==1)
         !
         IF ( nks > 1 ) CALL using_evc(2)
         IF ( nks > 1 ) CALL get_buffer ( wfc0, nwordwfc, iunwfc, ik) !SJ This calls davcio
         !
         IF ( nspin == 2 ) THEN
            !
            ...
            ispin = isk(ik)
            filename = TRIM(dirname) // 'wfc_shirley' // updw(ispin) // &
                 & TRIM(int_to_char(ik_g))
            !
         ELSE
            !
            ispin = 1
            filename = TRIM(dirname) // 'wfc_shirley_ob' // TRIM(int_to_char(ik_g))
            !
         ENDIF
         !
         ...
         IF ( my_bgrp_id == root_bgrp_id ) WRITE(stdout,*) 'npw_g',npw_g
         IF ( my_bgrp_id == root_bgrp_id ) WRITE(stdout,*) 'size(wfc0)',size(wfc0,1),size(wfc0,2)
         IF ( my_bgrp_id == root_bgrp_id ) WRITE(stdout,*) 'size(evc)',size(evc,1),size(evc,2)
         IF ( my_bgrp_id == root_bgrp_id ) WRITE(stdout,*) 'ngk(ik)',ngk(ik)
         IF ( my_bgrp_id == root_bgrp_id ) WRITE(stdout,*) 'ntot_e',ntot_e
         IF ( my_bgrp_id == root_bgrp_id ) CALL write_wfc( iunpun, &
              filename, root_bgrp, intra_bgrp_comm, ik_g, tpiba*xk(:,ik), &
              ispin, nspin, wfc0, npw_g, gamma_only, ntot_e, &
              igk_l2g_kdip(:), ngk(ik), tpiba*bg(:,1), tpiba*bg(:,2), &
              tpiba*bg(:,3), mill_k, 1.D0 )
         IF ( my_bgrp_id == root_bgrp_id ) WRITE(stdout,*) 'Completed writing optimal basis',filename
         !
      ...
      RETURN
      !
    END SUBROUTINE write_collected_wfc_simple
    !
    !------------------------------------------------------------------------
    SUBROUTINE write_collected_wfc_nkp( )
      !------------------------------------------------------------------------
      !
           USE kinds, ONLY : DP
     USE input_simple
     ...
     USE io_files,  ONLY : prefix, tmp_dir, iunpun
     USE fft_base,         ONLY : dffts
     ...
     USE uspp_init,        ONLY : init_us_2
      ...
      USE io_files,             ONLY : restart_dir, iunwfc, nwordwfc
      USE cell_base,            ONLY : tpiba, alat, bg
      USE control_flags,        ONLY : gamma_only, smallmem, lmanual_shirley_grid !SJ
      ...
      USE wavefunctions, ONLY : evc
      USE klist,                ONLY : nks, nkstot, xk, ngk, igk_k !ngk = #PW for each kpt
      USE input_parameters,     ONLY : qk, nksinterp_shirley !SJ
      USE gvect,                ONLY : ngm, g, mill, gcutm
      USE fft_base,             ONLY : dfftp
      USE wvfct,                ONLY : npwx, et, wg, nbnd !,ecutwfc
      USE lsda_mod,             ONLY : nspin, isk, lsda
      USE mp_pools,             ONLY : intra_pool_comm, inter_pool_comm, &
                                       npool,nproc_pool,me_pool
      USE mp_bands,             ONLY : me_bgrp, root_bgrp, intra_bgrp_comm, &
                                       root_bgrp_id, my_bgrp_id
      USE clib_wrappers,        ONLY : f_mkdir_safe
      !
      ...
      !SJ
      USE wfc_basis,            ONLY : wfc0
      USE input_simple,         ONLY : npw_max, ntot_e, lshrink
      USE gvecw,                ONLY : gcutw
      USE pwcom,                ONLY : init_igk_simple
      !USE deallocate_fft
      ...
      !
     IMPLICIT NONE
     INTEGER :: ii,jj,kk,kindex, sum_kindex
     INTEGER :: nksinterp
     REAL(kind=DP),ALLOCATABLE :: qkshirley(:,:) ! qkshirley(3,nksinterp) in 2pi/alat.
     REAL(DP), ALLOCATABLE :: gk(:)
     INTEGER               :: ios, ig, ngg, ipol, ispin, nbnd_collect
     INTEGER               :: ik, ik_g, ike, iks, npw_g
     INTEGER, EXTERNAL     :: global_kpoint_index
     INTEGER, EXTERNAL     :: n_plane_waves !SJ
     INTEGER,  ALLOCATABLE :: ngk_g(:), mill_k(:,:)
     INTEGER,  ALLOCATABLE :: igk_l2g(:), igk_l2g_kdip(:),  igk_l2g_kdip_gamma(:)
     CHARACTER(LEN=2), DIMENSION(2) :: updw = (/ 'up', 'dw' /)
     CHARACTER(LEN=256)    :: dirname
     CHARACTER(LEN=320)    :: filename
     CHARACTER(LEN=6), EXTERNAL :: int_to_char
     INTEGER               ::  igwx,igwx_gamma, iecutnew, npw_gamma,npw_stored
     INTEGER, ALLOCATABLE  :: igk_gamma(:), igk_l2g_npw(:,:)
     real(dp),allocatable :: g2kin_gamma(:)
     integer,allocatable :: igk_l2g_ecut(:,:)
     integer,allocatable :: igk_l2g_gamma(:) !,igk_l2g2(:)
     REAL(DP)              :: bglen, ecutnew, xk_gamma(3,1), real_dummy
     INTEGER :: integer_dummy

     !Need xk for write_collected_wfc. xk is in cartesian coordinate in units 2pi/alat (tpiba*xk when calling)
     !This should be equivalent to internal xk of QE. We use xk from crystal coordinate now from the input
     !Need to convert to 2pi/alat
         write(stdout,*) '------------------------------------------------------------'
         write(stdout,*) 'Begin k-point wfc files generation using SHIRLEY OB'
         write(stdout,*) '------------------------------------------------------------'     

     IF ( lmanual_shirley_grid  ) THEN
         write(stdout,*) '------------------------------------------------------------'
         write(stdout,*) 'kpoints read in from K_POINTS_SIMPLE card'
         write(stdout,*) '------------------------------------------------------------'
         nksinterp = nksinterp_shirley
         allocate(qkshirley(3,nksinterp))
         qkshirley = qk
         !CALL cryst_to_cart( nksinterp, qk, bg, 1 
         !do ii = 1,nksinterp
         !WRITE(stdout,*) 'KPOINT in crystal', ii, qk(:,ii)
         !WRITE(stdout,*) 'KPOINT in cart: 2pi/alat', ii, qkshirley(:,ii)
         !enddo
         !CALL cryst_to_cart( nksinterp, qk, bg, 1 )
     ELSE
         write(stdout,*) '------------------------------------------------------------'
         write(stdout,*) 'kpoints from uniform grid nkpoints(3)'
         write(stdout,*) '------------------------------------------------------------'
     !Generating uniform k-grid
         nksinterp = (nkpoints(1))*(nkpoints(2))*(nkpoints(3))
     ...
     do ii=0,nkpoints(1)-1
          do jj=0,nkpoints(2)-1
             do kk=0,nkpoints(3)-1
                  kindex = kk + jj*nkpoints(3) + ii*nkpoints(2)*nkpoints(3)+1
                  sum_kindex = kk+1
                  write(stdout,*) 'k-point' ,kindex
                  !
                  qkshirley(:,kindex) = bg(1:3,1)*dble(ii)/dble(nkpoints(1))+bg(1:3,2)*dble(jj)/dble(nkpoints(2))+&
    ...
    !Block for using nbnd or ntot_e
    nbnd_collect=ntot_e
    IF ( lshrink ) THEN
            WRITE(stdout,*) 'Collect optimal basis adjusted to the size of input wfc nbnd'
            nbnd_collect=nbnd
    ENDIF

    !Determine new ecut
    !ecutnew = ecutwfc  / tpiba2
    ecutnew = gcutw
  ...
  ! note that in case the lattice vectors have not been normalized
    ...
    !allocate( igk_gamma(npw_max), g2kin_gamma(npw_max) )
    ! ALLOCATE ( igk_l2g_kdip_gamma( npw_max ) )

    !if( allocated(igk_l2g_gamma) ) deallocate( igk_l2g_gamma )
    !allocate( igk_l2g_gamma(npw_max) )
    !Get new npw_g
    !write(stdout,*) 'Calling gk_sort for new igwx'
    !CALL gk_sort( xk_gamma(1,1), ngm, g, gcutw, npw_gamma, igk_gamma, g2kin_gamma )
    !write(stdout,*) 'Done calling gk_sort'
    !write(stdout,*) 'Calling gk_l2gmap_kdip'
    !write(stdout,*) 'ngm,ngk(1),npw_gamma gcutw', ngm,ngk(1),npw_gamma,gcutw
    !write(stdout,*) 'Size of igk_gamma', size(igk_gamma)
    !CALL gk_l2gmap( ngm,ig_l2g(1), npw_gamma, igk_gamma, &
    !                          igk_l2g_kdip_gamma )
    !write(stdout,*) 'Done calling gk_l2gmap_kdip'
    !write(stdout,*) 'Size of igk_l2g_kdip_gamm',size(igk_l2g_kdip_gamma)
    !igwx=maxval(igk_l2g_kdip_gamma)
    !call mp_max(igwx, intra_pool_comm)

    !WRITE(stdout,*) 'igwx',igwx
    !WRITE(stdout,*) 'ecutnew gcutm', ecutnew,gcutm
         !n_plane_waves( gcutw, nks, xk, g, ngm )
         ! n_plane_waves (ecutwfc, tpiba2, nks, xk, g, ngm, npwx, ngk)
    !WRITE(stdout,*) 'npwx before n_plane_waves',npwx
    !call n_plane_waves (gcutw, nksinterp, qkshirley, g, ngm, npwx)
    !WRITE(stdout,*)  'npwx after n_plane_waves',npwx
    !if( allocated( igk_gamma ) ) deallocate( igk_gamma )
    !allocate( igk_gamma(npwx) )
    !if( allocated( g2kin_gamma ) ) deallocate( g2kin_gamma )
    !allocate( g2kin_gamma(npwx) )
    !if( allocated( igk_l2g_npw ) ) deallocate( igk_l2g_npw )
    !allocate( igk_l2g_npw(npwx,nksinterp) )
    !igk_l2g_npw=0
    
    WRITE(stdout,*) 'size(ig_l2g)', size(ig_l2g,1)
    !xk_gamma =0.d0 
    !do ik=1,nksinterp
     !WRITE(stdout,*) 'ik',ik
     !call gk_sort(xk_gamma(1,1), ngm, g, gcutw, npw_gamma, igk_gamma, g2kin_gamma)
    ! call gk_sort(qkshirley(1,ik), ngm, g, gcutw, npw_gamma, igk_gamma, g2kin_gamma)
    ! call gk_l2gmap (ngm, ig_l2g(1), npw_gamma, igk_gamma, igk_l2g_npw(1,1))
    !enddo
     ...
     !WRITE(stdout,*) 'npw_g',npw_g
     !WRITE(stdout,*) 'ngk_g(ik)', ngk_g(ik)
    !igwx=maxval(igk_l2g_npw)
    !call mp_max( igwx,intra_pool_comm )
    
    !
    npw_gamma = 0
    integer_dummy = npw_max
    
    WRITE(stdout,*) 'npw_gamma',npw_gamma
    WRITE(stdout,*) 'npwx',npwx
    WRITE(stdout,*) 'npw_max', integer_dummy
    !WRITE(stdout,*) 'igwx from interp kgrid',igwx
    WRITE(stdout,*) 'ngm',ngm
    !WRITE(stdout,*) 'ecutnew gcutm', ecutnew,gcutm
    ...
    !do while ( igwx_gamma < igwx  .and. ecutnew <= gcutm )
    !  WRITE(stdout,*) 'loop',iecutnew
    !  call gk_sort(xk_gamma(1,1),ngm,g,ecutnew,npw_gamma, igk_gamma, g2kin_gamma)
    !  WRITE(stdout,*) 'Done with gk_sort'
    !  CALL gk_l2gmap( ngm, ig_l2g, npw_gamma, igk_gamma, &
    !                          igk_l2g_kdip_gamma )
    !  WRITE(stdout,*) 'Done with gk_l2gmap'
    !  if( igwx_gamma==0 ) npw_stored = npw_gamma
    !  igwx_gamma = maxval(igk_l2g_kdip_gamma(1:npw_gamma))
    !  write(stdout,*) 'igwx_gamma',igwx_gamma
    !  call mp_max(igwx_gamma, intra_pool_comm)
    !  iecutnew=iecutnew+1
    !  ecutnew = ecutnew + 1.d0*bglen
    !  write(stdout,*) 'i, igwx_gamma, igwx, old ecut, new ecut, npw_gamma'
    !  write(stdout,*) iecutnew, igwx_gamma, igwx, gcutw, ecutnew, npw_gamma
    !enddo

    !WRITE(stdout,*) 'Finished first loop'
    ...
    do while (npw_gamma < npw_max)!( igwx_gamma < igwx  .and. ecutnew <= gcutm )
      ...
      WRITE(stdout,*) 'Calling mp_max'
      !do ii=1,size(igk_l2g_kdip_gamma)
      !WRITE(stdout,*) 'igk_l2g_kdip_gamma(',ii,')',igk_l2g_kdip_gamma(ii)
      !enddo
      call mp_max(igwx_gamma, intra_pool_comm)
      iecutnew=iecutnew+1
      ecutnew = ecutnew + 1.d0*bglen
      !write(stdout,*) 'i, igwx_gamma, igwx, old ecut, new ecut, npw_gamma'
      !write(stdout,*) iecutnew, igwx_gamma, igwx, gcutw, ecutnew, npw_gamma
       write(stdout,*) 'i, igwx_gamma, old ecut, new ecut, npw_gamma'
       write(stdout,*) iecutnew, igwx_gamma,  gcutw, ecutnew, npw_gamma
    enddo
    ecutnew = ecutnew - 1.d0*bglen
    WRITE(stdout,*) 'New ecut', ecutnew
    
    !if( allocated( igk_gamma ) ) deallocate( igk_gamma ) !deallocate( igk_gamma )
    !if( allocated( igk_gamma ) ) deallocate( g2kin_gamma ) !deallocate( g2kin_gamma )
    !if( allocated( igk_gamma ) ) deallocate( igk_l2g_kdip_gamma ) !DEALLOCATE ( igk_l2g_kdip_gamma )
    ...
    !CALL realspace_grids_init(at,bg,gcutm,gcutms) gcut is updated with new ecut

    WRITE(stdout,*) 'Calling data_structure'
    CALL data_structure_simple(ecutnew,gamma_only) !ecutnew is taken here and gcutm is updated
    WRITE(stdout,*) 'Calling allocate_fft()'
    CALL allocate_fft()

    !CALL ggen( new ecut) gcutm should be updated now
    WRITE(stdout,*) 'Calling ggen'
    CALL ggen( dfftp, gamma_only, at, bg, gcutm, ngm_g, ngm, &
       g, gg, mill, ig_l2g, gstart, no_global_sort = smallmem )
    CALL ggens( dffts, gamma_only, at, g, gg, mill, gcutms, ngms )
     
    WRITE(stdout,*) 'ecutnew,gcutw', ecutnew,gcutw
    WRITE(stdout,*) 'gcutm/4',gcutm/4.0D0
    WRITE(stdout,*) 'ngm',ngm
    WRITE(stdout,*) 'New g size:',size(g,1),size(g,2)

    integer_dummy = n_plane_waves (gcutw, nksinterp, qkshirley, g, ngm)
    WRITE(stdout,*) 'Old n_plane_waves with gcutw  : ',integer_dummy
    integer_dummy = n_plane_waves (ecutnew, nksinterp, qkshirley, g, ngm)
    WRITE(stdout,*) 'New n_plane_waves with ecutnew: ',integer_dummy
    !Begin G vectors stuff
      !
      !
      !
      !
      ...
      !WRITE(stdout,*) 'Total number of kpoints in interpolate grid',nksinterp
      !WRITE(stdout,*) 'npw_max from simple', npw_max
      !WRITE(stdout,*) 'ngm',ngm

      iks = global_kpoint_index (nksinterp, 1)
      ike = iks + nks - 1
      IF (ALLOCATED(igk_k)) DEALLOCATE( igk_k )
      ALLOCATE( igk_k(npw_max+1,nksinterp) )  
      !We need to generate igk_k. Allocation is done in init_igk in Module klist in pwcom.f90
      IF (ALLOCATED(gk)) DEALLOCATE( gk )
      ALLOCATE ( gk(npw_max+1) )
      IF (ALLOCATED(ngk)) DEALLOCATE( ngk )
      ALLOCATE(ngk(nksinterp)) !Original ngk should be nsktot. Need to update this.
      !We need to generate igk_k. Allocation is done in init_igk in Module klist in pwcom.f90

      
      DO ik=1,nksinterp
       WRITE(stdout,*) 'Calling gk_sort for interp k-grid at ik',ik
       WRITE(stdout,*) 'K-POINTS:',qkshirley(:,ik)
       CALL gk_sort_simple( qkshirley(1,ik), ngm, g, ecutnew, ngk(ik), igk_k(1,ik), gk )
       !WRITE(stdout,*) 'igk_k',igk_k(1:5,ik)
      ENDDO
      WRITE(stdout,*) 'Size of ngk',size(ngk)
      ...
      ! ... ngk_g: global number of k+G vectors
      !
      ALLOCATE( ngk_g( nksinterp ) ) !nks = #kpt in pool
      ngk_g(1:nksinterp) = ngk(1:nksinterp) !Might need to modify ngk to simple PW
      CALL mp_sum( ngk_g, intra_bgrp_comm)
      CALL mp_bcast(npw_max,ionode_id,world_comm) 
      WRITE(stdout,*) 'In write_wfc'
      WRITE(stdout,*) 'intra_bgrp_comm world_comm'
      WRITE(stdout,*) intra_bgrp_comm, world_comm
      
      !Try to print npw_max in each proc
      !mp_barrier
      !
      ! ... The igk_l2g array yields the correspondence between the
      ! ... local k+G index and the global G index
      ...
      !
      WRITE(stdout,*) 'Allocating igk_l2g'
      
      CALL mp_barrier( intra_bgrp_comm )
      CALL mp_barrier( world_comm)
      WRITE(*,*) 'ionode_id,ionode,me_bgrp'
      WRITE(*,*) ionode_id,ionode,me_bgrp
      WRITE(*,*) 'npool,nproc_pool,me_pool'
      WRITE(*,*) npool,nproc_pool,me_pool
      WRITE(*,*) 'my_bgrp_id,root_bgrp_id,me_bgrp,root_bgrp'
      WRITE(*,*) my_bgrp_id,root_bgrp_id,me_bgrp,root_bgrp

      

      ALLOCATE ( igk_l2g( integer_dummy ) )
      !
      ! ... the igk_l2g_kdip local-to-global map yields the correspondence
      ! ... between the global order of k+G and the local index for k+G.
      !
      WRITE(stdout,*) 'Done allocating igk_l2g'
      CALL mp_barrier( intra_bgrp_comm )
      WRITE(stdout,*) 'Allocating igk_l2g_kdip'
      ALLOCATE ( igk_l2g_kdip( npw_max+1 ) )
      WRITE(stdout,*) 'Done allocating igk_l2g_kdip'
      CALL mp_barrier( intra_bgrp_comm )
      !
      WRITE(stdout,*) 'Done allocating igk_l2g_kdip'
      ALLOCATE ( mill_k( 3, npw_max+1 ) )

      WRITE(stdout,*) 'Size of mill(:,:)', size(mill,1),size(mill,2),size(mill)
      WRITE(stdout,*) 'Size of ig_l2g(:)', size(ig_l2g)
      !
      !WRITE(stdout,*) 'nks',nks
      k_points_loop: DO ik = 1, nksinterp
         !SJ OB is for all the kpoints.
         !SJ To get wavefunction at specific kpoint with wavevector k
         !SJ u_nk(x) = summation [1 -> ntot_e ] of [b_i_{nk}* b_i(x) ]
         WRITE(stdout,*) 'Working on kpoint ik:', ik
         ...
         ! ... npw_g is the maximum G vector index among all processors
         !
         npw_g = MAXVAL( igk_l2g(1:ngk(ik))) !ngk(ik)) )
         WRITE(stdout,*) 'npw_g',npw_g
         WRITE(stdout,*) 'ngk_g(ik)', ngk_g(ik)
         ...
         CALL gk_l2gmap_kdip( npw_g, ngk_g(ik), ngk(ik), igk_l2g, & !npw_max replaced ngk(ik)
                              igk_l2g_kdip )
         !
         ! ... mill_k(:,i) contains Miller indices for (k+G)_i
         !
         ...
         IF ( nks > 1 ) CALL using_evc(2)
         !IF ( nks > 1 ) CALL get_buffer ( wfc0, nwordwfc, iunwfc, ik) !SJ This calls davcio. 
                                                                      !This shouldn't be ik dependent?
         IF ( nks > 1 ) CALL get_buffer ( wfc0, nwordwfc, iunwfc, 1)
         !
         IF ( nspin == 2 ) THEN
            !
            ! ... LSDA: spin mapped to k-points, isk(ik) tracks up and down spin
         ...
         IF ( my_bgrp_id == root_bgrp_id ) WRITE(stdout,*) 'size(evc)',size(evc,1),size(evc,2)
         IF ( my_bgrp_id == root_bgrp_id ) WRITE(stdout,*) 'ngk(ik)',ngk(ik)
         IF ( my_bgrp_id == root_bgrp_id ) WRITE(stdout,*) 'nbnd_collect',nbnd_collect
         IF ( my_bgrp_id == root_bgrp_id ) CALL write_wfc( iunpun, &
              filename, root_bgrp, intra_bgrp_comm, ik_g, tpiba*qkshirley(:,ik), &
              ispin, nspin, wfc0, npw_g, gamma_only, nbnd_collect, &
              igk_l2g_kdip(:), ngk(ik), tpiba*bg(:,1), tpiba*bg(:,2), &
              tpiba*bg(:,3), mill_k, 1.D0 )
         IF ( my_bgrp_id == root_bgrp_id ) WRITE(stdout,*) 'Completed writing optimal basis',filename,'.dat'
         !
      END DO k_points_loop
      !
      DEALLOCATE ( mill_k )
      ...
      RETURN
      !
    END SUBROUTINE write_collected_wfc_nkp
    !
    !------------------------------------------------------------------------
    SUBROUTINE write_collected_wfc( )
      ...
      !SJ
      !WRITE(stdout,*) 'ngk_g before mp_sum', ngk_g
      CALL mp_sum( ngk_g, intra_bgrp_comm)
      !WRITE(stdout,*) 'ngk_g after mp_sum', ngk_g
      !SJ
      !
      ! ... The igk_l2g array yields the correspondence between the
      ! ... local k+G index and the global G index
    ...
    SUBROUTINE gk_l2gmap_kdip( npw_g, ngk_g, ngk, igk_l2g, igk_l2g_kdip, igwk )
      ...
      !SJ
      WRITE(stdout,*) 'In gk_l2gmap_kdip: npw_g,ngk_g,ngk', npw_g, ngk_g, ngk
      WRITE(stdout,*) 'In gk_l2gmap_kdip: igk_l2g(ngk)', igk_l2g(ngk)
      WRITE(stdout,*) 'In gk_l2gmap_kdip: Size of igk_l2g(ngk)', size(igk_l2g)
      !
      itmp(:)  = 0
      igwk_(:) = 0
      !
      ...
      END DO
      !
      !WRITE(stdout,*) 'ngg ngk_g',ngg,ngk_g !SJ
      IF ( ngg /= ngk_g ) THEN!&
         WRITE (stdout,*) 'ngg ngk_g',ngg,ngk_g
