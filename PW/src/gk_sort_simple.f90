...
!
!----------------------------------------------------------------------------
SUBROUTINE gk_sort_simple( k, ngm, g, ecut, ngk, igk, gk )
   !----------------------------------------------------------------------------
   ...
   USE input_simple,         ONLY : npw_max !SJ
   !
   IMPLICIT NONE
   !
   ...
   INTEGER,  INTENT(OUT) :: igk(npw_max)
   !! the correspondence k+G <-> G
   REAL(DP), INTENT(OUT) :: gk(npw_max)
   !! the moduli of k+G
   !
   !  ... local variables
   !
 ...
 REAL(DP) :: q   ! |k+G|^2
   ...
   DO ng = 1, ngm
      ...
      IF ( q <= ecut ) THEN
         ngk = ngk + 1
         IF ( ngk > npw_max ) THEN
            WRITE(6,*) 'ngk,npw_max', ngk,npw_max
            WRITE(6,*) 'Ecut:', ecut, 'gives ngk > npw_max'
            !ngk = ngk -1 !SJ Go back to previous ngk so that ngk = npw_max?
            RETURN
