...
SUBROUTINE newd( )
  ...
  !
!  WRITE(stdout,*) 'Calling newd'
!  WRITE(stdout,*) '.NOT. okvan', .NOT. okvan
  IF ( .NOT. okvan ) THEN
     !
...
#endif
  !IF (ionode) THEN
  !        WRITE(stdout,*) 'writing deeq'
  !        OPEN(unit = 9999,file='deeq.txt',status='unknown')
  !   DO ii=1,nhm
  !   DO jj=1,nhm
  !   DO kk=1,nat
  !        WRITE(9999,*) 'deequp(',ii,',',jj,',',kk,')',deeq(ii,jj,kk,1)
  !        WRITE(9999,*) 'deeqdw(',ii,',',jj,',',kk,')',deeq(ii,jj,kk,2)
  !   ENDDO
  !   ENDDO
  !   ENDDO
  !        CLOSE(9999)
