...
subroutine dielectric_spin(sh,input,kpnts,energy)
  !------------------------------------------------------------------------
  !
  ...
  !
  call start_clock('dielectric_spin')
  !
  ...
  write(stdout,*) ' '
  write(stdout,*) 'Computing the band structure... for down spin'
  write(stdout,*) ' '
  ...
  do ik=1,energy%nk_loc
     ...
     !SJ this should already be created previously in the spin up routine.
     ...
     if ( input%lcollect_bnk ) then
       ...
       filename = '.bnk_dw' // TRIM(int_to_char(iglobal_k))
     ...
     ENDIF
     iuneig=find_free_unit()
     filename= TRIM(int_to_char(iglobal_k))//'dw.txt'
  ...
  IF (input%build_unk) THEN
          WRITE(stdout,*) 'Building u_nk from diagonalized matrix and wfc from simple.x'
          ...
          DEALLOCATE(wfc)
          !CALL write_wfc(iglobal_k,input,npol,nbnd,igwx,tpiovera*q,unk,1)
          CALL write_wfc(iglobal_k,input,npol,nbnd,igwx,tpiovera*q,unk,nbnd_save,2)
  ...
  !
  !
end subroutine dielectric_spin
