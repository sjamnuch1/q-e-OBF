...
subroutine wfc_basis_hack
  ...
  COMPLEX(kind=DP), ALLOCATABLE :: wfc1(:,:),wfc3(:,:) !wfc0(:,:),wfc1(:,:),wfc3(:,:)
  !COMPLEX(DP), ALLOCATABLE, TARGET :: &
  !wfc0(:,:)
...
!Diagonalizing the current obf to check for eigen.
!         write(stdout,*) 'Diagonalization of basis:',ik
!         allocate(rwork(7*ntot_e),iwork(5*ntot_e),ifail(ntot_e))
!         allocate(work(1))
!         allocate(energies_tmp(ntot_e))
!         allocate(dmat(ntot_e,ntot_e))
!         lwork = -1
!         abstol = -1.d0
!         vl = 0.d0
!         vu = 0.d0
...
!         high_eig = ntot_e
!
!         CALL ZHEEVX('V','I', 'U', ntot_e, omat, ntot_e, vl, vu, low_eig, high_eig, abstol, &
!             & nwork, energies_tmp, dmat, ntot_e, work, lwork, rwork, iwork, ifail, info)
...
!
       ...
       !call ZGEMM('C','N',ntot_e,ntot_e,ntot_e,(1.d0,0.d0),dmat,ntot_e,dmat,ntot_e,(0.d0,0.d0),zmat,ntot_e)
       !   call mp_sum(zmat,world_comm)

       !   do ii=1,ntot_e
       !      do jj=1,ntot_e
       !         write(stdout,*) 'ORTHONORMALITY of eigenvectors (ii,jj):', ii,jj, zmat(ii,jj)
       !      enddo
       !   enddo

       !write(stdout,*) 'Done with printing eigenvectors of OBF*OBF at ', ik

       !write(stdout,*) 'Norm of basis'

       !do i=1,ntot_e
       ! norm = zdotc(npw_max*npol,wfc0(1,i),1,wfc0(1,i),1)
       ! call mp_sum(norm,world_comm)
       ! write(stdout,*) 'ik: norm(i)', ik, i, norm
       !enddo


       !deallocate(dmat,zmat,omat, rwork, work, iwork, ifail, energies_tmp)
!recalculate bec's
      !deallocate(omat)
...
end subroutine wfc_basis_hack




SUBROUTINE optimal_gram_schmidt_z(num_in,wfcs,ithres,thres,ik,num_out)
