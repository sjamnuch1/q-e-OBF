...
!----------------------------------------------------------------------------
  ...
  PRIVATE
  PUBLIC :: obf_write_schema
  !
  CONTAINS
    !------------------------------------------------------------------------
    SUBROUTINE obf_write_schema( only_init, wf_collect )
      !------------------------------------------------------------------------
      !
      ...
      USE input_basis,          ONLY : npw_gamma , ntot_e
      !
      IMPLICIT NONE
      !
      ...
      INTEGER ,ALLOCATABLE  :: ngk_g_init_band(:)
      REAL(DP) , ALLOCATABLE    :: wk_init(:)
      INTEGER               :: ngk_g !ngk_g(:) !OBF is only at gamma point. ngk_g now holds info for OBF
      ...
      real(dp) :: xk_gamma(3,1)=0.d0
      integer :: nks_obf=1
      REAL(DP), ALLOCATABLE :: et_tmp(:,:),wg_tmp(:,:)
      ...
      !
      npwx_g = ngk_g !MAXVAL(ngk_g(1:nkstot) )
      ngk_g_init_band(1) = ngk_g
      !
      ! XML descriptor
      ! 
      IF ( ionode ) THEN  
...
!-------------------------------------------------------------------------------------
         ...
         END IF
            CALL qexsd_init_k_points_ibz(qexsd_start_k_obj, k_points, calculation, &
                 1, 1, 1, 0, 0, 0, 1, alat, at(:,1), .TRUE., xk_gamma, wk_init)
         ...
         END IF 
         !  
         WRITE(stdout,*) 'lgauss', lgauss
         WRITE(stdout,*) 'Size of et', size(et,1),size(et,2)
         WRITE(stdout,*) 'Size of wg', size(wg,1),size(wg,2)
         WRITE(stdout,*) 'nks', nks
         WRITE(stdout,*) 'nkstot', nkstot
         WRITE(stdout,*) 'lsda', lsda
         !WRITE(stdout,*) 'wg(1:10,1)', wg(1:10,1)

         !Here if we are using single k-point. The et array will not have data up to ntot_e since
         !its size is only ndft bands. This leads to the code hanging so we now will use et_tmp.
         !Unknown eigenvalues are then set to zero and will be obtain during diagonalization step.
         IF ( ntot_e > size(et,1) ) THEN
                 IF (lsda) THEN !For up/down spin the et -> is (nbnd*2) x nks. Goes from up then down.
                     allocate(et_tmp(ntot_e,2))
                     allocate(wg_tmp(ntot_e,2))
                 ELSE
                     allocate(et_tmp(ntot_e,1))
                     allocate(wg_tmp(ntot_e,1))
                 ENDIF
         ELSE !This prevents the code from hanging during the double OBF run if ntot_e < second DFT bands.
                 IF (lsda) THEN
                     allocate(et_tmp(size(et,1),2))
                     allocate(wg_tmp(size(et,1),2))
                 ELSE
                     allocate(et_tmp(size(et,1),1))
                     allocate(wg_tmp(size(et,1),1))
                 ENDIF
         ENDIF
         et_tmp=0.d0
         !et_tmp(1:size(et,1),1)=et(1:size(et,1),1)
         

         !Do the same for occupation which is stored in wg
         !allocate(wg_tmp(ntot_e,1))
         wg_tmp=0.d0
         !wg_tmp(1:size(wg,1),1)=wg(1:size(et,1),1)

         IF (lsda) THEN
                 !et_tmp(1:size(et,1),2)=et(1:size(et,1),1)
                 !wg_tmp(1:size(et,1),2)=wg(1:size(et,1),1)
                 nks_obf=2
         ENDIF

         !WRITE(1111,*) et
         WRITE(stdout,*) 'Calling qexsd_init_band_structure'
         WRITE(stdout,*) 'nks_obf',nks_obf
            
         CALL qexsd_init_band_structure(  output_obj%band_structure,lsda,noncolin,lspinorb, nelec, natomwfc, &
                                 et_tmp, wg_tmp, nks_obf, xk_gamma, ngk_g_init_band, wk_init, SMEARING = smear_obj_ptr,  &
                                 STARTING_KPOINTS = qexsd_start_k_obj, OCCUPATIONS_KIND = qexsd_occ_obj, &
                                 WF_COLLECTED = .true., NBND = ntot_e, FERMI_ENERGY = ef_point, EF_UPDW = ef_updw,& 
                                 HOMO = h_energy_ptr, LUMO = lumo_energy ) !Might need to set WF_COLLECTED to true
         WRITE(stdout,*) 'Done with qexsd_init_band_structure'
         !WRITE(stdout,*) 'qexsd_occ_obj%occupations ' ,qexsd_occ_obj%occupations
         !WRITE(stdout,*) 'qexsd_input_obj%bands%occupations ' ,qexsd_input_obj%bands%occupations
         !WRITE(stdout,*) 'output_obj%band_structure%ks_energies', output_obj%band_structure%ks_energies
         ! 
...
!-------------------------------------------------------------------------------
    ...
    CONTAINS
       ...
       !
    END SUBROUTINE obf_write_schema
    !
    !
!----------------------------------------------------------------------------
END MODULE obf_xml_write
!----------------------------------------------------------------------------
