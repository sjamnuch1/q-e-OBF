...
!
!-----------------------------------------------------------------------
SUBROUTINE data_structure_resume(ecut, gamma_only, npw_obf )
  !-----------------------------------------------------------------------
  ...
  USE fft_base,   ONLY : dfftobf, fft_base_info, smap_obf
  ...
  !USE input_basis,     ONLY : qk, nksinterp_shirley
  IMPLICIT NONE
  !
  REAL(DP), INTENT(IN) :: ecut
  LOGICAL, INTENT(IN) :: gamma_only
  INTEGER, INTENT(IN) :: npw_obf
  !
  ! ... local variables
  !
  INTEGER :: ik, ngm_, ngs_
  LOGICAL :: lpara
  REAL(DP) :: gkcut_obf,gcutm_obf,gcutms_obf
  !
  lpara = ( nproc_bgrp > 1 )
  !
  !
  WRITE(stdout,*) 'ecut in data_structure_resume',ecut
  ...
  !
  gcutm_obf = ecut*4.D0 !Seem to need a factor of 4 to match the value in QE routine 
  !
  doublegrid = ( dual > 4.D0 )
  !
  IF ( doublegrid ) THEN
     !
     gcutms_obf = 4.D0 * ecut 
     !
  ELSE
     !
     gcutms_obf = gcutm
  ...
  CALL sticks_map_deallocate(smap_obf)
  ! task group are disabled if real_space calculation of calbec is used
  dfftobf%has_task_groups = (ntask_groups >1) .AND. .NOT. real_space
  ...
  WRITE(stdout,*) gkcut,gcutm_obf,gcutms_obf
  ...
  CALL fft_type_init( dfftobf, smap_obf, "rho" , gamma_only, lpara, intra_bgrp_comm, &
       at, bg, gcutm_obf , 4.d0       , fft_fact=fft_fact, nyfft=nyfft, nmany=nmany_, use_pd=pencil_decomposition_  )
  ! define the clock labels ( this enables the corresponding fft too ! )
  !dffts%rho_clock_label = 'ffts' ; dffts%wave_clock_label = 'fftw'
  dfftobf%rho_clock_label = 'fft'
  ...
  ngm_ = dfftobf%ngl( dfftobf%mype + 1 )
  !
  IF( gamma_only ) THEN
     !ngs_ = (ngs_ + 1)/2
     ngm_ = (ngm_ + 1)/2
  ENDIF
  WRITE(stdout,*) 'ngm_ in OBF', ngm_
  ...
  !
END SUBROUTINE data_structure_resume
