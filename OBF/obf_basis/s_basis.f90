...
!   deallocate(omat)
   ...
   IF ( restart_obf) THEN
           ...
           iloop=1
           WRITE(stdout,*) 'Nbasis from previous OBF run:', nbasis
           WRITE(stdout,*) 'DFT bands for this run', ntot
           npw_max = max(npw,npw_max)
           WRITE(stdout,*) 'npw_max during restart_obf' , npw_max
           allocate(wfc0(npw_max*npol,nbasis),wfc1(npw_max*npol,ntot_e))
           npw_work=npw_max*npol
           wfc0=(0.d0,0.d0)
           do ig = 1,npw_max
             WRITE(12000+mpime,*) 'maxval of igk_k(1:npw) ',MAXVAL(igk_k(1:npw,1))
             !WRITE(13000+mpime,*) 'igk_k',igk_k(ig,2)
           enddo
           !wfc0(1:npw_max*npol,1:nbasis)=wfc_e(1:npw_max*npol,1:nbasis)
           do ipol=0,npol-1
            do ig=1,npw
             !wfc0(igk_k(ig,1)+ipol*npw_max,1:ntot_e)
              IF ( igk_k(ig,1) > npw_max ) THEN
              !write(4000+mpime,*) 'igk_k',igk_k(ig,1)
                      CYCLE
              ENDIF
              wfc0(igk_k(ig,1)+npw*ipol,1:nbasis)=wfc_e(ig+npw*ipol,1:nbasis)
              !wfc0(ig+npw*ipol,1:nbasis)=wfc_e(ig+npw*ipol,1:nbasis)
            enddo
           enddo

           !Adjust the size of ntot_e so that it is updated to new OBF nbasis.
           !Normally this would be the same with ntot when starting which is
           !the DFT bands.
           ntot_e = nbasis

   ELSE ! Not restarting from previous OBF run. We have wfc0 from ik =1. 
        ! Then we start reading ik from 2 on.
           ...
           do ipol=0,npol-1
             ...
             enddo
           enddo
   ENDIF !Restart OBF or not loop


!loop on k
   do ik=iloop,nks!DEBUG should start from 2 for new run. 1 from using OBF
      ...
      !npw1 = ngk (ik)
      !npw = ngk (ik)
      write(stdout,*) 'Number of plane waves at K POINT: ', ik
      write(stdout,*) 'ngk(ik)', npw1

      !write(8000+mpime,*) 'ngk(ik)', npw1
      !write(8000+mpime,*) 'Size of evc',size(evc,1),size(evc,2)

      call davcio (evc, 2*nwordwfc, iunwfc, ik, -1)
      

      !wfc_t=(0.d0,0.d0) !wfc_t(npwx,ntot)
      !do ipol=0,npol-1
      !   wfc_t(1+ipol*npwx:npw+ipol*npwx,1:ntot)=evc(1+ipol*npwx:npw+ipol*npwx,num_nbndv(1)-num_val+1:num_nbndv(1)+num_cond)
      !enddo

      !Load the next kpoint wfc into working memory
      wfc1=(0.d0,0.d0)
      do ipol=0,npol-1
         do ig=1,npw1
...
!!calculate overlap
       ...
       call ZGEMM('C','N',ntot_e,ntot,npw_work,(1.d0,0.d0),wfc0,npw_work,wfc1,npw_work,(0.d0,0.d0),omat,ntot_e)
...
!!project out
       call start_clock('zgemm')
       call ZGEMM('N','N',npw_work,ntot,ntot_e,(-1.d0,0.d0),wfc0,npw_work,omat,ntot_e,(1.d0,0.d0),wfc1,npw_work)
...
!Check S matrix
       ...
       call ZGEMM('C','N',ntot,ntot,npw_work,(1.d0,0.d0),wfc1,npw_work,wfc1,npw_work,(0.d0,0.d0),dmat,ntot)
       call mp_sum(dmat,world_comm)
  ...
  !
       ...
       call ZGEMM('N','N',npw_work,nfound,ntot,(1.d0,0.d0),wfc1,npw_work,zmat(:,1:nfound),&
       ...
       allocate(wfc2(npw_work,ntot_e+nfound_gs))
       ...
       wfc2(1:npw_work,1:ntot_e)=wfc0(1:npw_work,1:ntot_e)
