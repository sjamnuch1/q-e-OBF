subroutine khamiltonian_spin
    ...
    !
    call start_clock('khamiltonian_spin')
    !
    IF (nspin .eq. 1) RETURN
    write(stdout,*) 'Spin polarized calculation. Repeat khamiltonian routine again for spin down case'
    ...
    if(ionode) then
      iun=find_free_unit()
      open( unit= iun, file=trim(tmp_dir)//trim(prefix)//'.hamiltonian_down', &
    ...
    g2e_mat = 0.d0
    !
    !WRITE(25000+mpime,*) 'dffts%nl(1:npw_max)',dffts%nl(1:npw_max)
    do ii=1,ntot_e
         psic(1:dffts%nnr)=0.d0
         psic(dffts%nl(1:npw_max))=evc(1:npw_max,ii)
    ...
    if (noncolin) then
     do ii=1,ntot_e
       if (domag) then
               ...
               do ir=1, dffts%nnr
                  v_rwfc(ir,ii) = v_rwfc(ir,ii) * vrs(ir,2)
                  v_rwfc(dffts%nnr+ir,ii) = v_rwfc(dffts%nnr+ir,ii) * vrs(ir,2)
               enddo
     ...
     enddo
      ...
      do ii=1,ntot_e
        do ir=1,dffts%nnr
          v_rwfc(ir,ii) = v_rwfc(ir,ii)*vrs(ir,2)  ! V_loc(r) x e_i(r) !SJ vrs(ir,1) -> vrs(ir,2) for opposite spin
        enddo
      enddo
    end if
    ...
    !do ii=1,ntot_e
    !do ir=1,dffts%nnr
    !write(40000+mpime,*) REAL(rwfc(ir,ii)),AIMAG(rwfc(ir,ii))
    !write(60000+mpime,*) REAL(v_rwfc(ir,ii)),AIMAG(v_rwfc(ir,ii))
    !enddo
    !enddo
    ...
    else !SJ change deeq(1:nhm,1:nhm,1:nat,1) ->  deeq(1:nhm,1:nhm,1:nat,2) for down spin
     !WRITE(stdout,*) 'size(deeq)',size(deeq,1),size(deeq,2),size(deeq,3),size(deeq,4)
     !WRITE(stdout,*) 'deeq(1,1,1,2)',deeq(1:nhm,1:nhm,1:nat,2)
     deeqc(1:nhm,1:nhm,1:nat) = cmplx(deeq(1:nhm,1:nhm,1:nat,2), 0.d0, KIND=DP)
     !write(stdout,*) 'deeqc_down(1,1,1)',deeqc(1,1,1)!SJ
     !DO ii=1,nhm
     !DO jj=1,nhm
     !DO kk=1,nat
     !WRITE(13000+mpime,*) 'deeq(',ii,',',jj,',',kk,')',deeq(ii,jj,kk,2)
     !ENDDO
     !ENDDO
     !ENDDO
    ...
    !
    write(stdout,*) 'For norm-conserving PP, the k-dependent nonlocal parts are the same. Skipping'
    !if(ionode) then
    !  iun1=find_free_unit()
    !  open( unit= iun1, file=trim(tmp_dir)//trim(prefix)//'.hamiltonian_k_down', &
    !            &status='unknown',form='unformatted')
    !endif
    !
    ...
    write(stdout,*) '**********************************'
    !
    call stop_clock('khamiltonian_spin')
    !
    ...
    call print_clock('khamiltonian_spin')
    !
end subroutine khamiltonian_spin
