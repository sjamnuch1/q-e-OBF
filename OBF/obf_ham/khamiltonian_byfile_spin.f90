subroutine khamiltonian_byfile_spin
     !------------------------------------------------------------------------------------------
     !
   ...
   WRITE(stdout,*) 'npw_max',npw_max
    ...
    !if (noncolin) then
    !  allocate(sum_beck_nc(nkb,npol,ntot_e))
    !else
    !   allocate(sum_beckc(nkb,ntot_e))
    !endif
    !
    !
    ...
    if (noncolin) then
     ...
     enddo
      ...
      do ii=1,ntot_e
        do ir=1,dffts%nnr
          v_rwfc(ir,ii) = v_rwfc(ir,ii)*vrs(ir,2)  ! V_loc(r) x e_i(r)
        enddo
      enddo
    end if
    ...
    !do ii=1,nksinterp_shirley
     ...
     !           if(ionode) then
     !             iun1=find_free_unit()
      !            open( unit= iun1, file=trim(tmp_dir)//trim(prefix)//'.hamiltonian_k'//TRIM(int_to_char(ii)), &
      !            &status='unknown',form='unformatted')
      !          endif
      ...
      !          call calbec(npw_max,vkb_max,evc,beck,ntot_e) ! scalar product <\beta_Ilm|exp(ik.r)|e_j>
                !
     !           if (noncolin) then
      !             sum_beck_nc(1:nkb,1:npol,1:ntot_e) = beck%nc(1:nkb,1:npol,1:ntot_e)
       !         else
        !           sum_beckc(1:nkb,1:ntot_e) = beck%k(1:nkb,1:ntot_e)
               ! endif
     ...
     !        if (noncolin) then
      !         if (ionode) write(iun1) sum_beck_nc(1:nkb,1:npol,1:ntot_e)
      ...
      !       else
      !         if (ionode) write(iun1) sum_beckc(1:nkb,1:ntot_e)
               !if (ionode .and. ii == 1) write(stdout,*) 'Writing sum_beckc: Size ', size(sum_beckc,1),&
                 !      size(sum_beckc,2),size(sum_beckc,3)
       !      endif
       !      close(iun1) !Need to close now otherwise the find_free_unit() will run out of integer
       !      call stop_clock('Vnloc_write')
             !
             !
    !enddo  !nksinterp_shirley Vnl_k loop

    !
    deallocate(deeqc,g2e_mat,g2e) !d_wfc_e,g2e)
    !if (noncolin) then
    ...
    !
end subroutine khamiltonian_byfile_spin
